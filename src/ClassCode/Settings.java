...
public class Settings {
    ...
    public String RegExCreat(String word)// 不使用筛选器
    {
        ...
        String expression;
        if (isCaps && isMatch)
            expression = "^" + word + "(\\..*)?$";// 区分大小写且全字匹配
        else if (isCaps && !isMatch)
            expression = ".*" + word + ".*";// 区分大小写且不全字匹配
        else if (!isCaps && isMatch)
            expression = "^(?i)" + word + "(\\..*)?$";// 不区分大小写且全字匹配
        else if (!isCaps && !isMatch)
            expression = ".*(?i)" + word + ".*";// 不区分大小写且不全字匹配
        else
            expression = ".*(?i)" + word + ".*";
